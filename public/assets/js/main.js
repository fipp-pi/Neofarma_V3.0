...
(function() {
  ...
  function initInputMasks() {
    ...
    // Phone number input mask (Brazil: (XX) 9 XXXX XXXX)
    ...
    if (phoneInput) {
      phoneInput.addEventListener('input', function(e) {
        ...
        const nono = value.slice(2, 3);
        const bloco1 = value.slice(3, 7);
        const bloco2 = value.slice(7, 11);
        ...
        if (value.length === 0) {
          ...
          e.target.value = '(' + ddd + ') ' + nono;
        } else if (value.length <= 7) {
          e.target.value = '(' + ddd + ') ' + nono + ' ' + bloco1;
        } else {
          e.target.value = '(' + ddd + ') ' + nono + ' ' + bloco1 + ' ' + bloco2;
        }
      });
    }
  ...
  // Formulário de login: envia email/senha para POST /login e redireciona se ok
  ...
  function validarLogin(event) {
    ...
    if (!usuario || !senha) {
      if (errorBox) {
        ...
        errorBox.classList.add('animate__animated', 'animate__shakeX');
        setTimeout(() => errorBox.classList.remove('animate__shakeX'), 1000);
      }
      return;
    }

    // Atalho admin (opcional)
    if (usuario === 'admin@email.com' && senha === 'admin') {
    ...
    fetch('/login', {
      ...
      body: JSON.stringify({ email: usuario, password: senha, redirect: redirectTo })
    })
      ...
      .then(function (result) {
        ...
        if (errorBox) {
          errorBox.textContent = result.data.message || 'Email ou senha inválidos.';
          errorBox.classList.remove('d-none');
          errorBox.classList.add('animate__animated', 'animate__shakeX');
          setTimeout(() => errorBox.classList.remove('animate__shakeX'), 1000);
        }
